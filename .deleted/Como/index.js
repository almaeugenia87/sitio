...
const Como = props => {
  ...
  const query = gql`{
    ...
    manuales(sort: "orden", filter: {tipo: {_eq: "uso"}}) {
      titulo
      descripcion
      imagen {
        id
  ...
  return (
    ...
    <main className={style.main}>
      ...
      <div className={style.grid}>
        ...
        { data && data.manuales.map((uso, i) => {

          return (
            <section className={style.section} key={i}>
              <Heading space="small" color="secondary" hue="light"><Title size="small">{uso.titulo}</Title></Heading>
              <div className={style.paragraph}>
                <Image className={style.image} src={`${context.api}/assets/${uso.imagen.id}`}/>
                <Text size="medium" align="justify">{uso.descripcion}</Text>
              </div>
            </section>)
          })
