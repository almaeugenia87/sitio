...
const Autocomplete = ({ collection, source, name, value, onSelect, disabled, ...other }) => {
  ...
  const selectHint = e => {
    ...
    source.setInput(source.hints[collection][e.currentTarget.value][source.searchField]);

    // Código adaptado de: https://stackoverflow.com/questions/23892547/what-is-the-best-way-to-trigger-change-or-input-event-in-react-js
    Object.getOwnPropertyDescriptor(window.HTMLInputElement.prototype, 'value').set.call(inputRef.current, source.hints[collection][e.currentTarget.value].id);
  ...
  useEffect(() => {
      ...
      if (value) source.getSelected({ variables: { id: value }}); //.then(data => source.setInput(data[collection].nombre)).catch(e => console.log(e));
  ...
  useEffect(() => {
      ...
      if (source.selected && source.selected[collection]) source.setInput(source.selected[collection].nombre);
  ...
  return (
    <div className={style.container} {...other}>
      ...
      { source.hints && source.hints[collection].length > 0 &&
        <ul className={style.hints} id="hints" >
          { source.hints[collection].map((hint, i) => (
